describe('microsoft', async function () {
    ...
    it('Тест на появление "гамбургера" при ширине менее 576px', async function ({
        browser,
    }) {
        const puppeteer = await browser.getPuppeteer();
        const [page] = await puppeteer.pages();
    ...
    it('Тест, что при выборе элемента из меню "гамбургера", меню должно закрываться', async function ({
        browser,
    }) {
        const puppeteer = await browser.getPuppeteer();
        const [page] = await puppeteer.pages();
